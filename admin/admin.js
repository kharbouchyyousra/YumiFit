/* ============================================================
   ADMIN — version API + JWT + Socket.io
   ============================================================ */
...
async function renderAdminProducts() {
  ...
  tbody.innerHTML = products.map(p => `
    <tr>
      ...
      <td style="color:#777;font-size:.9rem">${p.desc}</td>
    </tr>
  `).join('');
}
...
function initSocket() {
  ...
  socket.on('new-order', (order) => {
    ...
    showNotification(`🔔 Nouvelle commande #${order.number} — ${order.customer.prenom} ${order.customer.nom}`);
    refresh();
  });
...
function showNotification(msg) {
  let n = document.getElementById('admin-toast');
  if (!n) {
    n = document.createElement('div');
    n.id = 'admin-toast';
    n.style.cssText = `
      position:fixed;bottom:30px;right:30px;background:#2a9d8f;color:#fff;
      padding:16px 24px;border-radius:10px;box-shadow:0 6px 20px rgba(0,0,0,.2);
      font-weight:500;z-index:9999;transform:translateX(120%);transition:transform .3s;
    `;
    document.body.appendChild(n);
  }
  n.textContent = msg;
  n.style.transform = 'translateX(0)';
  setTimeout(() => n.style.transform = 'translateX(120%)', 4000);
}